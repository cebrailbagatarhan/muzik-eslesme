...
const consentVersions:Record<(typeof optionalKinds)[number],string>={
  'product-analytics':'2026-09-v1',
  'research-contact':'2026-09-v1',
};
export async function privacyRoutes(app:FastifyInstance,ctx:Context) {
  app.get('/v1/consents',async req=>{
    ...
    return {
      ...
      optional:optionalKinds.map(kind=>({kind,granted:active.has(kind),version:active.get(kind)?.version??consentVersions[kind]}))
    };
  });
  app.put('/v1/consents/:kind',async req=>{
    ...
    return ctx.write(req,'consent.updated',async q=>{
      ...
      return {kind,granted,version:consentVersions[kind]};
    });
  });
}
