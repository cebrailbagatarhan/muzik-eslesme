export type Config = {
  ...
  ageSecret: string; scannerUrl: string; scannerToken: string; rateMultiplier: number;
};
export function readConfig(env: NodeJS.ProcessEnv = process.env): Config {
  ...
  const c: Config = {
    ...
    redisUrl: env.REDIS_URL ?? '', origins: (env.ALLOWED_ORIGINS ?? 'http://localhost:8081').split(','),
    ...
    spotifyAllowlist: (env.SPOTIFY_ALLOWED_USER_IDS ?? '').split(',').filter(Boolean),
    ageSecret: env.AGE_WEBHOOK_SECRET ?? '', scannerUrl: env.PHOTO_SCANNER_URL ?? '',
    scannerToken: env.PHOTO_SCANNER_TOKEN ?? '', rateMultiplier: 1,
  };
