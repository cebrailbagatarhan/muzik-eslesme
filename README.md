...
- E-posta / parola ile üyelik, 18+ doğum tarihi kapısı, süreli oturum ve refresh token rotasyonu.
...
- Engelleme, şikâyet, şifreli kanıt saklama, moderatör rolü ve TOTP doğrulaması.
- Veri dışa aktarma, yeniden parola doğrulamasıyla hesap silme ve ilişkili kayıtların temizlenmesi.
...
Komutun verdiği TOTP anahtarını kimlik doğrulayıcı uygulamana ekle; paylaşma veya depoya kaydetme. API'yi tekrar açıp yeniden giriş yap. Moderasyon sekmesinde 6 haneli kodla 10 dakikalık yetkili oturum açılır. Kendi fotoğrafını veya taraf olduğun raporu onaylayamazsın.

## PostgreSQL ve dış servisler

API'nin `.env` dosyasında `DATABASE_URL` verilirse `pg` bağlantı havuzu kullanılır. Migration dosyaları sürüm tablosuyla sırayla uygulanır. `REDIS_URL` verilirse hız limitleri ve sunucular arası yenileme olayları Redis üzerinden çalışır. `APP_MODE=beta`, PostgreSQL, Redis, HTTPS origin, yaş doğrulama webhook anahtarı ve fotoğraf tarayıcısı yapılandırmasını zorunlu kılar.

Yaş sağlayıcısı `POST /v1/verification/age` adresine `eventId`, `userId`, `verified`, `timestamp` alanlarını gönderir. `x-verification-signature`, `eventId.userId.verified.timestamp` metninin `AGE_WEBHOOK_SECRET` ile HMAC-SHA256 hex imzasıdır. İstek 5 dakika içinde olmalı; olay kimliği tekrar işlenmez. Fotoğraf tarayıcısı, `PHOTO_SCANNER_URL` adresine gönderilen temizlenmiş JPEG için `{ "clean": true }` döndürmelidir; bearer kimliği `PHOTO_SCANNER_TOKEN` ayarıdır.
...
Halka açık kullanım için lisanslı katalog, bağımsız yaş doğrulama sağlayıcısı, e-posta doğrulama/parola kurtarma, işletmeye ait gizlilik metinleri, moderasyon operasyonu ve dağıtım yapılandırması tamamlanmalıdır. Fotoğraflar bu sürümde PostgreSQL'de şifreli tutulur; özel obje depolama adaptörü, push bildirimleri, ayrı Next.js yönetim paneli ve mağaza yayını dahil değildir. Moderasyon ekranı aynı Expo uygulamasının içindedir. Mesaj şifrelemesi sunucuda AES-GCM'dir, uçtan uca şifreleme değildir. Hesap silme canlı veritabanını temizler; işletim yedeklerinin imhası dağıtım sorumlusuna aittir.

Teslimden önce 26 otomatik test geçti ve web çıktısı derlendi. Tarayıcıda görsel inceleme tamamlanmadı. Testleri daha sonra çalıştırmak istersen:
